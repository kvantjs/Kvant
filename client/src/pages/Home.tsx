...
export default function Home() {
  ...
  return <div className="kvant-shell">
    <aside className={`kvant-sidebar ${sidebarCollapsed ? "collapsed" : ""} ${mobileNav ? "mobile-open" : ""}`}>
      ...
      <div className="sidebar-bottom"><NavItem icon={<Settings2 size={16} />} label="Configurações" /><button className="profile-row" onClick={() => isAuthenticated ? logout() : startLogin()}><span className="profile-avatar">{user?.name?.slice(0, 1).toUpperCase() ?? <CircleUserRound size={16} />}</span><span className="profile-copy"><strong>{authLoading ? "Carregando..." : user?.name ?? "Entrar no Manus"}</strong><small>{isAuthenticated ? "Sessão conectada" : "Login para salvar"}</small></span><MoreHorizontal size={16} /></button></div>
    </aside>

    <main className="kvant-main">
      <header className="topbar"><div className="topbar-left"><IconButton label="Abrir menu" onClick={() => setMobileNav(true)}><Menu size={18} /></IconButton><IconButton label={sidebarCollapsed ? "Mostrar sidebar" : "Ocultar sidebar"} onClick={() => setSidebarCollapsed((value) => !value)}>{sidebarCollapsed ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}</IconButton><div className="breadcrumbs"><span className="breadcrumb-muted">Workspace</span><ChevronRight size={14} /><span>{workspaceQuery.data?.name ?? "Kvant starter"}</span></div></div><div className="builder-toolbar"><button className="builder-mode-active" onClick={() => setActiveTab("preview")}><Eye size={14} /> Preview</button><button onClick={() => setActiveTab("code")}><Code2 size={14} /> Código</button><button onClick={() => setActiveTab("terminal")}><Database size={14} /> Dados</button><span className="builder-address">sandbox://workspace</span><IconButton label="Atualizar preview"><RefreshCw size={14} /></IconButton><IconButton label="Abrir preview em nova aba"><ExternalLink size={14} /></IconButton></div><div className="topbar-right"><button className="share-button"><Share2 size={13} /> Compartilhar</button><button className="publish-button"><UploadCloud size={13} /> Publicar</button><span className="connection-pill"><span className="status-dot" /> Manus runtime</span><IconButton label="Mais ações"><MoreHorizontal size={18} /></IconButton></div></header>
