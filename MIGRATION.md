# Migração do Kvant para Manus

## Resultado

O projeto `kvant-manus` adapta o núcleo do aplicativo Kvant para o runtime fullstack do Manus, mantendo a experiência de workspace em três áreas: navegação/histórico, chat de desenvolvimento e workbench de arquivos.

## Componentes portados

| Área do Kvant | Implementação no Manus |
|---|---|
...
| Login | Manus OAuth já fornecido pelo scaffold fullstack |
...
## Integrações Manus usadas

As chaves e chamadas de modelo permanecem no servidor por meio de `server/_core/llm.ts`; nenhum segredo do provedor é exposto ao navegador. Chats e workspaces autenticados são persistidos no banco Manus. O login utiliza a infraestrutura OAuth do scaffold, e o preview é servido pelo ambiente gerenciado do WebDev.
